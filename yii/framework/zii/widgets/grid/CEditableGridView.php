...
class CEditableGridView extends CGridView {
	...
	public function renderQuickBar() {
		$assets = Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('zii.widgets.assets')).'/ceditablegridview';
		$cs=Yii::app()->getClientScript();
		$cs->registerScriptFile($assets . '/CEditableGridView.js');
		...
		foreach($this->columns as $column) 
		{
			if(!$column instanceof CButtonColumn)
			{
				if($column instanceof CCheckBoxColumn)
					printf('<td class="%s"><input name="%s[%s][]" type="checkbox" /></td>', $column->tdCssClass, $this->dataProvider->modelClass, $column->name);
					...
					|| $column instanceof CEditableColumn)
				{
					...
					if(strstr($column->name, '.') != FALSE) // Column contains an relation
					{
						...
						if ($column->hidden == false) {
							printf('<td class="%s"><input name="%s[%s][]" type="text" class="%s" /></td>', $column->tdCssClass, $this->dataProvider->modelClass, $column->name, $column->inputCssClass);
						}
						else
						{
							printf('<td class="%s"><input name="%s[%s][]" type="hidden" class="%s" /></td>', $column->tdCssClass, $this->dataProvider->modelClass, $column->name, $column->inputCssClass);
						}
					}
				}
		...
		printf('<td><input type=button value="%s" id="addbtn_0" class="ceditable_grid_view_addbtn"></td>', $this->addButtonValue);
		echo "</tr>";
		echo "</form>";
