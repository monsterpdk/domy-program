...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	{
		$operation_lower = strtolower(Utils::atalakit_ekezet_nelkulire($operation));

		if($allowCaching && $params===array() && isset($this->_access[$operation_lower]))
			return $this->_access[$operation_lower];

		$access=Yii::app()->getAuthManager()->checkAccess($operation_lower,$this->getId(),$params);
		if($allowCaching && $params===array())
			$this->_access[$operation_lower]=$access;
