...
class ExportableGridBehavior extends CBehavior {
    ...
    private function csvRowHeaders($fileHandle, $attributes, CModel $model) {
		// a 0. sorba eltároljuk, hogy milyen objektumot is exportálunk (importnál használjuk majd)
		$row = array();
		$row[] = get_class($model);
			
		$first_row = array_merge($row, array_fill(1, count($attributes) - 1, ''));
		fputcsv($fileHandle, $first_row, $this->csvDelimiter, $this->csvEnclosure);
		
		$rows = array();
        $rowLabels = array();
		
		// LI: hozzáadunk egy első oszlopot, amiben jelezzük, hogy az adott sort
		// Új sor esetén nem töltendő,
		// M - módosítottuk,
		// T - töröltük.
		$rows[] = '';
		$rowLabels[] = 'M, ha módosult a rekord';

        foreach ($attributes as $attr) {
            $rows[] = $attr;
            $rowLabels[] = $model->getAttributeLabel($attr);
        }
		
        fputcsv($fileHandle, $rowLabels, $this->csvDelimiter, $this->csvEnclosure);
		
		// LI : ha valamiért szükség lenne az olvasható oszlopneveken kívül az attribútumra magára
        //fputcsv($fileHandle, $rows, $this->csvDelimiter, $this->csvEnclosure);
    }
