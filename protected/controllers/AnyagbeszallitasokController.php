...
//				die() ;
	...
	public function actionNyomdakonyvListazas ($termek_id, $nyomdakonyv_id = 0) {
		if ($termek_id != null) {
			$sql =
				"
					SELECT
						dom_megrendeles_tetelek.munka_neve AS munka_neve,
						dom_nyomdakonyv.id AS nyomdakonyv_id
					FROM dom_nyomdakonyv
					...
					WHERE dom_megrendeles_tetelek.hozott_boritek = 1 AND dom_nyomdakonyv.elkeszulesi_datum = CONVERT(0, DATETIME) AND termek_id = :termek_id	
				";			
			
			$command = Yii::app()->db->createCommand($sql);
			$command->bindParam(':termek_id', $termek_id);

			$nyomdakonyvek = $command -> queryAll();
			
			if ($nyomdakonyvek != null) {
				foreach ($nyomdakonyvek as $nyomdakonyv) {
					echo CHtml::tag('option', array('value'=>$nyomdakonyv['nyomdakonyv_id'], 'selected' => $nyomdakonyv_id == $nyomdakonyv['nyomdakonyv_id'] ? 'selected' : ''), CHtml::encode($nyomdakonyv['munka_neve']),true);
				}
			}
		}
	}
