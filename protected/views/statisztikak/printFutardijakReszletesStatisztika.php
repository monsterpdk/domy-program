...
<div style='text-align:center'>
	<h1> <span class='under'>Futárdíjak részletes statisztika</span> </h1>
</div>
...
<?php
	...
	echo '<tr>
		...
		$futarOsszesenSzallitasiDij = 0;
		$termekOsszesenSzallitasiDij = 0;
		...
		foreach ($dataProvider -> getData() as $item) {
			if ($actualFutarCeg == '' || $actualFutarCeg != $item['futar_ceg']) {
				if ($actualMegrendelo != '') {
					echo "<tr> <td align='left' style='padding-left:40px'>Összesen:</td> <td align='right'> <strong> " . Utils::DarabszamFormazas($termekOsszesenDb) . " </strong></td> <td align='right'> <strong> " . Utils::OsszegFormazas($termekOsszesenSzallitasiDij, 0) . " </strong> </td> </tr>";
				...
				if ($actualFutar != '') {
					echo "<tr> <td align='left' style='padding-left:20px'>Összesen:</td> <td align='right'> <strong> " . Utils::DarabszamFormazas($futarOsszesenDb) . " </strong></td> <td align='right'> <strong> " . Utils::OsszegFormazas($futarOsszesenSzallitasiDij, 0) . " </strong> </td> </tr>";
				...
				if ($actualFutarCeg != '') {
					echo "<tr> <td align='left'>Összesen:</td> <td align='right'> <strong> " . Utils::DarabszamFormazas($futarCegOsszesenDb) . " </strong></td> <td align='right'> <strong> " . Utils::OsszegFormazas($futarCegOsszesenSzallitasiDij, 0) . " </strong> </td> </tr>";
			...
			if ($actualMegrendelo == '' || $actualMegrendelo != $item['megrendelo']) {
				...
				$futarOsszesenSzallitasiDij += $item['szallitas_dij'];
				$termekOsszesenSzallitasiDij += $item['szallitas_dij'];
				$osszegOsszesen += $item['szallitas_dij'];
...
<p align='right'>
	<strong>Szállítás díja összesen: <?php echo Utils::OsszegFormazas($osszegOsszesen, 0); ?> Ft</strong>
</p>
