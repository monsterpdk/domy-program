...
<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'anyagbeszallitas-termekek-iroda-form',
...
//			$zarodasok[" "] = "Nincs" ;
		...
		<div class="row boritekMeretRadioGroup">
			<label>Termék kereső</label>
			 ...
			 <?php echo CHtml::Button('Termék', array('name' => 'search_termek_iroda', 'id' => 'search_termek_iroda', 'onclick' =>
												'
												var valasztott_meret = $("input[name=boritek_meret]:checked", "#anyagbeszallitas-termekek-iroda-form").val()
												var valasztott_zaras = $("input[name=boritek_zarodas]:checked", "#anyagbeszallitas-termekek-iroda-form").val()
												...
												$.updateGridView("termekek-iroda-grid' . $grid_id . '", paramKeys, paramValues);
	...
	<!-- CJUIDIALOG BEGIN -->
		...
		$this->widget('zii.widgets.grid.CGridView', 
		   array( 'id'=>'termekek-iroda-grid' . $grid_id,
								  ...
								  'value'=>'CHtml::Button("+", 
															array("name" => "send_termek", 
																...
																"onClick" => "$(\"#termek_dialog' . $grid_id . '\").dialog(\"close\");
																			  ...
																			  $(\"#AnyagbeszallitasTermekekIroda_termek_id\").val(\"$data->id\");
																			  $(\"#AnyagbeszallitasTermekekIroda_autocomplete_termek_name\").val(\"$data->nev\");
...
<script>
	...
	//$( "#AnyagbeszallitasTermekekIroda_rendelt_darabszam" ).on("keyup", keyPressEvent);
	
	// LI: arra az esetre, ha nem csak darabár, hanem nettó összeg is kell, egyelőre nem használjuk
	function osszegSzamol() {
		var darabszam = $.isNumeric ($( "#AnyagbeszallitasTermekekIroda_rendelt_darabszam" ).val()) ? $( "#AnyagbeszallitasTermekekIroda_rendelt_darabszam" ).val() : 0;
		var netto_darabar = $.isNumeric ($( "#AnyagbeszallitasTermekekIroda_netto_darabar" ).val()) ? $( "#AnyagbeszallitasTermekekIroda_netto_darabar" ).val() : 0;
		
		$( "#AnyagbeszallitasTermekekIroda_netto_darabar" ).val (Math.round(darabszam * netto_darabar));			
	}
	
	function keyPressEvent() {
		$( "#AnyagbeszallitasTermekekIroda_netto_darabar" ).val($( "#AnyagbeszallitasTermekekIroda_netto_darabar" ).val().replace(",",".")) ;
		$( "#AnyagbeszallitasTermekekIroda_netto_darabar" ).val("0") ;
		nettoar_kalkulal() ;
		//osszegSzamol() ;
	}
	
	function nettoar_kalkulal() {
		var termek_id = $("#AnyagbeszallitasTermekekIroda_termek_id").val() ;
	...
	function calculateTermekNettoDarabAr (termekId) {
		...
		<?php echo CHtml::ajax(array(
				'url'=> "js:'/index.php/AnyagbeszallitasTermekekIroda/calculateNettoDarabAr/ugyfel_id/' + ugyfel_id + '/termek_id/' + termekId + '/darabszam/' + darabszam",
				...
				{
					$('#AnyagbeszallitasTermekekIroda_netto_darabar' ).val (data);
				} ",
		)); ?>;
	}
	...
	function showNyomdakonyvSelector() {
		if ($("#AnyagbeszallitasTermekekIroda_hozott_boritek").prop("checked")) {
	...
	function termekValtozas () {
		...
		if ($("#AnyagbeszallitasTermekekIroda_hozott_boritek").prop("checked")) {
			if ($("#AnyagbeszallitasTermekekIroda_termek_id").val () != '') {
				<?php echo CHtml::ajax(array(
					'url'=> "js:'/index.php/Anyagbeszallitasok/nyomdakonyvListazas/termek_id/' + $('#AnyagbeszallitasTermekekIroda_termek_id').val() + '/nyomdakonyv_id/' + $('#nyomdakonyv_id_hidden').val()",
					'data'=> "",
					'type'=>'post',
					'id' => 'send-link-'.uniqid(),
					'success'=>"js:function(data)
					{
						...
						$('option', '#AnyagbeszallitasTermekekIroda_nyomdakonyv_id').not(':eq(0)').remove();
						
						$('#AnyagbeszallitasTermekekIroda_nyomdakonyv_id').append(data);
					}",
				)); ?>
			}
		}
