...
class CEditableColumn extends CGridColumn
{
	...
	protected function renderDataCellContent($row,$data)
	{
		$field = $this->name;
		if ($this->hidden) {
			printf('<input class="%s" name="%s[%s][]" type="hidden" value="%s" />', $this->inputCssClass, $data->tableSchema->name, $field, $data->$field);
		}
		else {
			printf('<input class="%s" name="%s[%s][]" type="text" value="%s" />', $this->inputCssClass, $data->tableSchema->name, $field, $data->$field);
		}
	}
}
